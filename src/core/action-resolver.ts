...
export function resolveAction(
  ...
  // Accept any positive integer (1, 2, … 10, 11, …) — single-digit-only here was
  // a latent bug: numbered lists past 9 entries (e.g. the gift picker) never
  // resolved two-digit taps, silently falling through to the raw-text fallback.
  if (/^[1-9]\d*$/.test(textLower) && lastMenu?.length) {
